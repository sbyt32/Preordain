    ...
    for card_data in res:
        ...
        with conn.transaction() as tx1:
            while keep_adding_cards:
                log.debug(
                    f"Getting {card_name} data with offset of {offset_value}, recieved {resp['resultCount']} results."
                )
                for sale_data in resp["data"]:
                    ...
                    try:
                        with conn.transaction() as tx2:
                            log.debug(
                                f"Attempting to insert into table card_data_tcg with data | order date: {order_date}, cond: {condition}, variant: {variant}, qty: {qty}, sale $: {buy_price}, ship $: {ship_price}"
                            )
                        ...
                        if (
                            ...
                            or isoparse(order_date) > isoparse(config.TCG_SALES)
                        ):
                            log.warning(
